...
/// For applicative, see Validation.
...
module Result =
    ...
    /// Concat a nested Result type - equivalent to Result.bind id
    let concat (r: Result<Result<'Success, 'Error>, 'Error>): Result<'Success, 'Error> =
        r |> Result.bind id
